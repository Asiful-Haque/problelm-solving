...
int main(){
	...
	while(t--){
		ll n;cin>>n;
		ll x;string s;
		ll zero_one=INT_MAX,one_zero=INT_MAX,one_one=INT_MAX;
		for(ll i=0;i<n;i++){
			cin>>x;
			cin>>s;

			if(s=="01")zero_one=min(x,zero_one);
			else if(s=="10")one_zero=min(x,one_zero);
			else if(s=="11")one_one=min(x,one_one);
		}
		
		if(one_zero!=INT_MAX && zero_one!=INT_MAX){
			cout<<min((one_zero+zero_one),one_one)<<endl;
		}
		else if(one_zero==INT_MAX || zero_one==INT_MAX){
			if(one_one!=INT_MAX){
				cout<<one_one<<endl;
			}
			else{
